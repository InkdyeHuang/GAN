...
def build_generator(z_piror,y_condition):
    
    g_input = tf.concat([z_piror,y_condition],3) # g_input_size= [batch_size,1,1,z_size + y_size]
    
    input_channel = int(g_input.shape[3])
    
    W_1 = tf.Variable(tf.truncated_normal([7,7,64,input_channel],stddev = 0.2), dtype = tf.float32)
    B_1 = tf.Variable(tf.constant(0.0,shape=[64]), dtype = tf.float32)
    H_1 = tf.nn.relu(tf.nn.conv2d_transpose(g_input,W_1,[batch_size,7,7,64],strides = [1,1,1,1],padding='VALID') + B_1)
    
    W_2 = tf.Variable(tf.truncated_normal([5,5,32,64],stddev = 0.2), dtype = tf.float32)
    B_2 = tf.Variable(tf.constant(0.0,shape = [32]), dtype = tf.float32)
    H_2 = tf.nn.relu(tf.nn.conv2d_transpose(H_1,W_2,[batch_size,14,14,32],strides = [1,2,2,1],padding='SAME') + B_2)
    
    W_3 = tf.Variable(tf.truncated_normal([5,5,1,32],stddev = 0.2), dtype = tf.float32)
    ...
    x_generator = tf.nn.sigmoid(H_3)#激活函数？
    
    g_params = [W_1,B_1,W_2,B_2,W_3,B_3]
    
    return x_generator,g_params

def build_discrimitor(x_real,x_generator,y_condition):
    
    X = tf.concat([x_real,x_generator],0)
    
    W_1 = tf.Variable(tf.truncated_normal([5,5,1,32],stddev = 0.2), dtype = tf.float32)
    B_1 = tf.Variable(tf.constant(0.0,shape=[32]), dtype = tf.float32)
    H_1 = tf.nn.relu(tf.nn.conv2d(X,W_1,strides=[1,2,2,1],padding='SAME') + B_1)
    ...
    W_3 = tf.Variable(tf.truncated_normal([7,7,64,y_size],stddev = 0.2), dtype = tf.float32)
    B_3 = tf.Variable(tf.constant(0.0,shape=[y_size]), dtype = tf.float32)
    H_3 = tf.nn.relu(tf.nn.conv2d(H_2,W_3,strides=[1,1,1,1],padding='VALID') + B_3)
    
    H_3_flat = tf.reshape(H_3,[-1,y_size])
    ...
    W_output = tf.Variable(tf.truncated_normal([y_size*2,1],stddev = 0.2), dtype = tf.float32)
...
def train(to_restore):
    ...
    z_sample = np.random.normal(0,1,size = (batch_size,1,1,z_size)).astype(np.float32)
    ...
    y_condition_1 = tf.placeholder(tf.float32,[None,1,1,y_size])
    y_condition_2 = tf.placeholder(tf.float32,[None,y_size])
    
    x_generator,g_params = build_generator(z_piror,y_condition_1)
    
    y_real,y_generator,d_params = build_discrimitor(x_real,x_generator,y_condition_2)
    ...
    for epoch in range(max_epochs):
        for i in range(int(x_sample_size // batch_size)):
            ...
            y_sample_2 = y_sample_d[batch_size * i : batch_size * (i+1)]/255.
            x_sample_each = x_sample_each/255.
            _,d_l = sess.run([d_trainer,d_loss],feed_dict = {x_real:x_sample_each,z_piror:z_sample,y_condition_1:y_sample_1, y_condition_2:y_sample_2})
            _,g_l = sess.run([g_trainer,g_loss],feed_dict = {x_real:x_sample_each,z_piror:z_sample,y_condition_1:y_sample_1, y_condition_2:y_sample_2})
            print('epoch: %s,i:%s, d_loss: %s, g_loss: %s'%(epoch,i,d_l,g_l))
        if epoch % 5 == 0:
            x_generator_vaule = sess.run(x_generator,feed_dict = {z_piror : z_sample,y_condition_1:y_sample_1})
            show_image(x_generator_vaule,'./output/cgan_%s.jpg'% int(epoch//5))
            saver.save(sess,'./output/cgan.model')
    sess.close()
def test(size):
    ...
    y_test = one_hot([1])
    x_gen_val = sess.run(x_generator, feed_dict={z_prior: z_test_value, y_condition_1 :y_test})
